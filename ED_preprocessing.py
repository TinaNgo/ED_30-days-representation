...
# True: re-admitted to ED within 30 days
# False: not re-admitted to ED within 30 days
# Died: died during stay, or within 30 days of being discharged from the ED without re-admission
	...
	for i in range (len(admissions)):
		...
		if i < len(admissions) - 1:
			...
			if admissions.loc[i, 'subject_id'] == admissions.loc[i + 1, 'subject_id']:
				# if re-admitted within 30 days
				if (admissions.loc[i + 1, 'intime'] - admissions.loc[i, 'outtime']).days <= 30:
				...
				if pd.notna(admissions.loc[i, 'dod']):
					# if die within 30 days of discharged of discharged with no more ED re-admission
					if (admissions.loc[i, 'dod'] - admissions.loc[i, 'outtime']).days <= 30:
						re_admitted_or_died.append("DIED")
					else: # not die within 30 days of discharged
						re_admitted_or_died.append(False)
				# no day of death recorded		
				else:
					re_admitted_or_died.append(False)
		
		else: # last row, only need to check dod
			if pd.notna(admissions.loc[i, 'dod']):
				# if die within 30 days 
				if (admissions.loc[i, 'dod'] - admissions.loc[i, 'outtime']).days <= 30:
					re_admitted_or_died.append("DIED")
				else: # not die within 30 days of discharged
					re_admitted_or_died.append(False)
...
def main():
	...
	# removed entries where the patient died within 30 days or during the stay
